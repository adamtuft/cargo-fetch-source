...
use crate::git::{GitSource, GitReference};
...
mod test {
    ...
    fn test_git_clone_task_progress() {
        ...
        let mut tasks: Vec<_> = spawn_tasks(&sources, PathBuf::from("test/test_git_clone_task_progress"))
    ...
    fn test_git_clone_subprocess() {
        let sources = get_sources();
        let mut tasks: Vec<_> = spawn_tasks(&sources, PathBuf::from("test/test_git_clone_subprocess"))
            .collect::<Result<_, _>>().unwrap();
